...
Codex CLI 在 `wire_api = "responses"` 模式下支持 `model_reasoning_effort` 配置。代理会按上游目标模型自动把 `reasoning_effort` 透传为合适的参数：
...
- 若上游模型不识别 `reasoning_effort`（例如部分 MiniMax 模型），代理会保守跳过，避免 400 错误。
...
5. 如需推理效果，确认上游目标模型支持 `reasoning_effort`；代理会在遇到 400 时自动 failover 降级重试。
