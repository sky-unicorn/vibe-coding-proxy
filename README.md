...
- **管理员账号/密码**：`[admin]` 段是管理员账户的唯一事实来源。每次启动都会校准 `admin_users` 表使其与之一致 —— 修改密码或用户名后重启即生效（改用户名时旧账户会被删除）。修改完成后用新账号/新密码登录即可。
- 若需修改监听 `host`（如仅本机访问）或使用 `gunicorn` / `waitress` 等 WSGI 服务器，再编辑 `app.py` 文件末尾的 `app.run(...)`。
...
- **首次启动会自动创建默认管理员账号**：用户名 `admin`，密码 `admin123`（每次 `init_db` 在该表为空时自动写入，**首启即存在弱口令，请登录后立即在管理界面或数据库中修改**）
...
2. **API Key 安全**：管理员账号与 API Key 均以哈希 / 随机串形式存于 SQLite，请妥善保管 `proxy.db`。每次 `init_db` 在 `admin_users` 表为空时都会自动创建默认管理员 `admin / admin123`，**首启即存在弱口令**，务必登录后立即在管理界面或数据库中修改。
...
> 以下为**管理员内部运维脚本**，仅用于手动创建或重置管理员账号，普通使用者无需关心。

每次启动时 `init_db()` 都会按 `proxy.ini` 的 `[admin]` 段校准管理员账户（未配置时为 `admin / admin123`），修改后重启即生效，通常无需手动重置。如确需绕过 proxy.ini 手动写入，可执行以下内部脚本（通过数据库连接写入管理员表，密码经哈希后存储）：
